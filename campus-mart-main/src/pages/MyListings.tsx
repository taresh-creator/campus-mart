...
export function MyListings({ onNavigate, onEdit }: MyListingsProps) {
  ...
  async function confirmDelete() {
    ...
    try {
      await api.listings.delete(deleteTarget.id);
  ...
  async function toggleStatus(listing: Listing, e: React.MouseEvent) {
    e.stopPropagation();
    const newStatus = listing.status === 'active' ? 'sold' : 'active';
    try {
      const updated = await api.listings.updateStatus(listing.id, newStatus);
      setListings((prev) =>
        prev.map((l) =>
          l.id === listing.id ? { ...l, status: updated.status } : l
        )
      );
    } catch (err) {
            ...
            <div key={listing.id} className="relative group">
              ...
              <div className="absolute bottom-3 left-3">
                <button
                  type="button"
                  onClick={(e) =>
                    toggleStatus(listings.find((l) => l.id === listing.id)!, e)
                  }
                  title={`Click to mark as ${
                    listing.status === 'active' ? 'Sold' : 'Active'
                  }`}
                  className={`px-2.5 py-1 rounded-full text-xs font-semibold cursor-pointer transition-all hover:shadow-sm ${
