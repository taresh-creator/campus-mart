        ...
        http
                ...
                .exceptionHandling(exceptions -> exceptions
                        .authenticationEntryPoint((request, response, authException) -> {
                            ...
                            response.getWriter().write("{\"status\":401,\"message\":\"Unauthorized: Please sign in to continue.\"}");
                        })
                )
                .authorizeHttpRequests(auth -> auth
                        // Public Auth endpoints
                        .requestMatchers(HttpMethod.POST, "/api/auth/signup", "/api/auth/login").permitAll()
                        // Public image upload retrieval
                        .requestMatchers(HttpMethod.GET, "/api/uploads/**").permitAll()
                        // Public listing views
                        .requestMatchers(HttpMethod.GET, "/api/listings").permitAll()
                        // Specific listings endpoint matching:
                        // my-listings MUST be authenticated:
                        .requestMatchers(HttpMethod.GET, "/api/listings/my-listings").authenticated()
                        // Individual listing view is public:
